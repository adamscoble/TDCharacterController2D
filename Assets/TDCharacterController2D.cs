...
public class TDCharacterController2D : MonoBehaviour {
    ...
    public void Move(Vector2 motion) {
        Move(motion, out Vector2 delta);
    }

    /// <summary>Attempts to move the controller by <paramref name="motion"/>, the motion will only be constrained by collisions. It will slide along colliders. The final movement of the controller is stored in <see cref="delta"/>.</summary>
    public void Move(Vector2 motion, out Vector2 delta) {
        Vector3 currentPosition = Transform.position + (Vector3)_collider.offset;
        Vector3 newPosition = GetNewPosition(currentPosition, motion);
        newPosition.z = currentPosition.z;

        delta = newPosition - currentPosition;

        if (delta.sqrMagnitude < _sqrMinimumMovementThreshold) {
            delta = Vector2.zero;

            return;
        }

        Transform.position = newPosition - (Vector3)_collider.offset;
    }
